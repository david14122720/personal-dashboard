...
**Scope.** The subscription model becomes "mensual fijo" (D2): create/edit moves to Settings with `name`, `price` and a monthly due date; `frequency` is forced to `monthly` in the UI and stays vestigial in the database; a dedicated `POST /subscriptions/{id}/pay` action debits a chosen account and records an audit movement atomically; the paid state is computed lazily from `last_paid_on`/`next_billing_on` with no cron; the category kind gate is removed from writes (D3). The fixed "Suscripciones" movement category is seeded with an idempotent collision rule. Slice S-B depends on S-A (`finance-movements`).

**Open items resolved here.** (a) Pay is a dedicated endpoint, not a client orchestration of PATCH + POST (explore §5 Q5). (b) The seed collision rule is exact-key idempotent upsert with user rows untouched (see the seed requirement). (c) `PATCH` is widened to `name`/`price`/`next_billing_on`/`is_active` only.

**Edge cases.** A free subscription (`price = 0.00`) exists by contract ("Free tiers supported") but cannot be paid because movements require `amount > 0`; the UI hides Pay and the BE answers 422 — this is the explicit resolution of the D2 × D4 conflict recorded in the proposal. A subscription left unpaid for months advances to the first future occurrence, never one month into the past. Deleting a subscription keeps its movements as audit via `ON DELETE SET NULL`.

**Non-goals.** No scheduler/worker (no cron anywhere), no credit-card statement-cycle revival, no transfers, no custom categories in the subscription category select (customs stay localStorage-only), no new dependencies.
