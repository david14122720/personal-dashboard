...
export default function PendingTasks() {
  ...
  if (rows.length === 0) return <div><EmptyState title={t("productivity.noTasks")} /><a href="/dashboard/productivity/">{t("dashboard.viewInProductivity")}</a></div>;
  return <div><ul className="flex flex-col gap-2">{rows.map((r) => <li key={r.id} className="flex justify-between gap-3 rounded-lg border border-hull px-3 py-2 text-sm"><span className="truncate">{r.title}</span><span className="shrink-0 font-mono text-xs tabular-nums text-instrument/60">{r.due_date ?? ""}</span></li>)}</ul><a href="/dashboard/productivity/" className="mt-2 inline-block text-xs text-signal underline-offset-2 hover:underline">{t("dashboard.viewInProductivity")}</a></div>;
}
