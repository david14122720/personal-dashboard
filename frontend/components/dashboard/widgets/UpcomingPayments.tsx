...
import { useEvents, useSubscriptions } from "@/lib/api/dashboard";
...
export default function UpcomingPayments() {
  ...
  if (subs.isLoading || events.isLoading) return <div role="status">{t("common.loading")}</div>;
  if (subs.error || events.error) return <div role="alert"><p>{t("dashboard.loadFailed")}</p><button type="button" onClick={() => void mutate((k) => typeof k === "string" && k.startsWith("dashboard/"))}>{t("common.retry")}</button></div>;
  const items = toUpcomingPayments(subs.data, events.data).slice(0, 7);
